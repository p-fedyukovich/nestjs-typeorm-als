...
describe('TypeOrm session', () => {
  ...
  it('should create user and purse', async () => {
    ...
    const purse1 = await request(server).post(`/purse/${userId}`).send({balance: 0});
  ...
  it('should create user and two purses', async () => {
    ...
    const userId = userRes.body.id;
    const purse1 = await request(server).post(`/purse/${userId}`).send({balance: 0});
    const purse2 = await request(server).post(`/purse/${userId}`).send({balance: 0});
  ...
  it('should create user and one purse and top-up it', async () => {
    const userRes = await request(server).post('/').send({ name: 'Pavel' });
    const userId = userRes.body.id;
    const purse = await request(server).post(`/purse/${userId}`).send({balance: 0});
    const purseId = purse.body.id;
    const purseWithBalance = await request(server).patch(`/purse/${purseId}/top-up`).send({sum: 100});
  ...
  it('should create user two users with purses and make remittance', async () => {
    ...
    let purse1 = await request(server).post(`/purse/${userId1}`).send({balance: 100});
    let purse2 = await request(server).post(`/purse/${userId2}`).send({balance: 100});
